                    ...
                    previewImg.onload = () => {
                        const updateSize = () => {
                            ...
                            if (wmDrag) {
                                ...
                                wmCanvas.width = metrics.width + 4; 
                ...
                downloadImage: async (e) => {
                    ...
                    const termsHtml = `
                        <div class="text-left text-sm text-gray-600 space-y-2">
                            <p>Bạn bắt buộc phải chủ động <a href="/user/${app.currentPhoto.profiles.id}" onclick="event.preventDefault(); app.ui.closeAlert(false); app.utils.navigate('/user/${app.currentPhoto.profiles.id}')" class="text-blue-600 hover:underline font-bold">liên hệ</a> và nhận được sự đồng ý bằng văn bản từ tác giả gốc trước khi tái sử dụng ảnh cho bất kỳ mục đích nào. Nếu sử dụng trái phép khi chưa có sự cho phép, tác giả có toàn quyền gửi yêu cầu gỡ bỏ vi phạm bản quyền hoặc thực hiện các biện pháp khiếu nại pháp lý liên quan.</p>
