    ...
    async then(resolve, reject) {
        ...
        try {
            ...
            if (this.method === "insert") {
                ...
                } else if (this.table === "photos") {
                    let id = this.params.id || this.params.eq_id || this.params._id;
                    res = await app.api.del("/api/photos/" + id);
                } else if (this.table === "admin_notes") {
                    let id = this.params.id || this.params.eq_id || this.params._id;
                ...
                if (this.table === "admin_notes") {
                    ...
                    let id = this.params.id || this.params.eq_id || this.params._id;
                    res = await app.api.put("/api/photos/" + id, this.actionData);
                } else if (this.table === "profiles") {
                    let id = this.params.id || this.params.eq_id || this.params._id;
                    res = await app.api.put("/api/auth/users/" + id, this.actionData);
                    ...
                    toast.innerHTML = `
                        <div class="relative w-8 h-8 flex justify-center items-center shrink-0">
                            <svg class="w-full h-full transform -rotate-90 absolute inset-0" viewBox="0 0 120 120">
                                <circle class="stroke-gray-200 dark:stroke-gray-700" cx="60" cy="60" r="54" fill="none" stroke-width="12" />
                                <circle id="${toastId}-circle" class="transition-all duration-300 ease-out stroke-black dark:stroke-white" cx="60" cy="60" r="54" fill="none" stroke-width="12" stroke-dasharray="339.29" stroke-dashoffset="339.29" stroke-linecap="round" />
                            </svg>
                            <div class="absolute inset-0 z-10 flex justify-center items-center pointer-events-none">
                                <div class="w-[14px] h-[14px] overflow-hidden flex justify-center items-center mask-y-fade">
                                    <i class="fa-solid fa-up-long text-[13px] text-gray-800 dark:text-white upload-arrow-anim text-center"></i>
                                </div>
                            </div>
                        </div>
                        <div class="flex-1 overflow-hidden pointer-events-none select-none">
...
closeCustomRolePrompt: () => {
    ...
    maintenance: {
                ...
                fetch: async () => {
      try {
          const statusData = await app.api.get('/api/system/status');
          if (statusData && statusData.banned) {
              const reasonText = statusData.reason || 'Vi phạm quy định của VNBUSARCHIVE';
              const banReason = `IP hoặc tài khoản của bạn đã bị cấm với lý do: <b>${reasonText}</b>`;
      ...
      } catch (e) { if(e.message !== "Banned IP") console.error("Lỗi lấy thông tin bảo trì", e); }
  },
                  check: (sysId) => {
                ...
                compressToSizeLoop: async (imageSource, targetMime = 'image/webp', targetKB = 500) => {
                    ...
                    if (!hasWebpNative) {
                        try {
                            ...
                            maxQ = 95;
                            while (maxQ - minQ >= 2) {
                                let midQ = Math.round((minQ + maxQ) / 2);
                                let webpBuffer = await encode(imageData, { quality: midQ, use_sharp_yuv: 1 });
                            ...
                            if (!bestBlob) {
                                let webpBuffer = await encode(imageData, { quality: 70, use_sharp_yuv: 1 });
                                let wasmBlob = new Blob([webpBuffer], { type: 'image/webp' });
                    ...
                    // báo lỗi và không tiếp nhận thay vì xuất JPEG lặng lẽ
                    if (hasWebpNative) {
                    ...
                    // Fallback sang JPEG (trình duyệt không hỗ trợ WebP và WASM cũng lỗi)
                    minQ = 0.70;
                    maxQ = 0.95;
                    while (maxQ - minQ >= 0.02) {
                        let midQ = (minQ + maxQ) / 2;
