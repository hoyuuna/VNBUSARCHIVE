...
node build-core.js
```
Lệnh này sẽ gộp các file trong `src/js/` thành `public/app.js` (plain text), render `public/index.html` từ `_core.html`, sinh `public/_headers` từ `csp.json`, và biên dịch Tailwind CSS. **Tuyệt đối không** chỉnh sửa trực tiếp `public/app.js` hoặc `public/index.html` (đây là file được sinh tự động), và không nhúng script logic thủ công vào `_core.html`.
...
- **Tái sử dụng UI:** Tuân thủ hệ thống thiết kế HoyuUI. **Luôn tái sử dụng** các component và token thiết kế hiện có (xem quy tắc trong `.agents/AGENTS.md`). Không tự ý tạo CSS component mới nếu không có sự đồng ý.
- **Modal Alerts:** BẮT BUỘC dùng `app.ui.showAlert()` thay cho `alert()` hay `confirm()` mặc định của trình duyệt.

### Bảo mật (Security)
- Tham khảo kỹ `.agents/security.md`.
- **Không tự ý thay đổi Content-Security-Policy (CSP)** trong `csp.json` trừ khi thêm API/CDN hợp lệ (sau đó chạy lại `node build-core.js` để sinh `public/_headers`).
