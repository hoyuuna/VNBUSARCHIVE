export async function onRequest(context) {
    ...
    let operatorId = params.id;
    try { operatorId = decodeURIComponent(params.id); } catch(e) {}
    
    const url = new URL(request.url);
    const indexReq = new Request(url.origin + '/', request);
    const response = await env.ASSETS.fetch(indexReq);
    
    let title = `${operatorId} | VNBUSARCHIVE`;
    let desc = `Xem các hình ?nh phuong ti?n thu?c don v? ${operatorId} trên VNBUSARCHIVE.`;
