                            ...
                            try {
                                ...
                                if (rData) {
                                    ...
                                    rData.forEach(p => {
                                        if (p.route_no && p.route_no !== 'Khác' && p.route_no !== 'Không rõ') {
                                            ...
                                            if (!uniqueRoutesMap.has(key)) {
                                                uniqueRoutesMap.set(key, p.route_no);
                                            }
                                        }
                                    });
                                    ...
                                    if (finalRoutes.length > 0) {
                                        const { data: rtInfo } = await window.sb.from('route_info').select('route_name, short_path').in('route_name', finalRoutes);
                                        if (rtInfo) rtInfo.forEach(rt => { shortPaths[rt.route_name.toLowerCase()] = rt.short_path; });
                                    }
                                    for (const r of finalRoutes) {
                                        let displayR = app.utils.cleanText(r);
                                        let sp = shortPaths[r.toLowerCase()];
                                        if (sp) displayR += \` (\${app.utils.cleanText(sp)})\`;
                                        const routeUrl = '/route/' + encodeURIComponent(r);
                                        routeCards.push(\`
