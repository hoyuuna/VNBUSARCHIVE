window.callApiSearch = async (action, payload = {}) => {
    try {
        const { data: { session } } = await window.sb.auth.getSession();
        const response = await fetch('/api/search', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action, payload, token: session?.access_token })
        });
                ...
                triggerMainSuggestion: async (query, inputId = 'search-input', sugId = 'main-search-suggestions') => {
                    ...
                    app.suggestionTimeouts[inputId] = setTimeout(async () => {
                        ...
                        try {
                            ...
                            const fetchSugs = async (sugType, label) => {
                                try {
                                    const { data: { session } } = await window.sb.auth.getSession();
                                    const response = await fetch('/api/search', {
                                        method: 'POST',
                                        ...
                                        body: JSON.stringify({
                                            action: 'fetch_sugs',
                                            payload: { sugType, label, searchWords, currentPreference: app.preference.current },
                            ...
                            if (filter === 'all') {
                                const [plates, routes, ops, models] = await Promise.all([
                                    fetchSugs('vehicles_plate', 'BKS'),
                                    fetchSugs('photos_route', 'Tuyến'),
                                    fetchSugs('photos_operator', 'Đơn vị vận hành'),
                                    fetchSugs('vehicles_model', 'Dòng xe')
                                ]);
                                ...
                                results = await fetchSugs('vehicles_plate', 'BKS');
                            } else if (filter === 'route' || filter === 'absolute_route') {
                                results = await fetchSugs('photos_route', 'Tuyến');
                            } else if (filter === 'operator') {
                                results = await fetchSugs('photos_operator', 'Đơn vị vận hành');
                            } else if (filter === 'model') {
                                results = await fetchSugs('vehicles_model', 'Dòng xe');
                            } else if (filter === 'location') {
                                results = await fetchSugs('photos_location', 'Vị trí');
                            } else if (filter === 'camera') {
                                results = await fetchSugs('photos_camera', 'Thiết bị');
                            } else if (filter === 'uploader') {
                                results = await fetchSugs('profiles_username', 'Người đăng');
                            }
                ...
                try {
                    ...
                    if (filterType === 'uploader' || filterType === 'all') {
                        cardPromises.push((async () => {
                            try {
                                const { data: usersData } = await window.callApiSearch('fetch_uploader_cards', { searchWords });

                                if (usersData && usersData.length > 0) {
                                    for (const user of usersData) {
                                        const { count } = await window.callApiSearch('fetch_uploader_photo_count', { userId: user.id });
                    ...
                    if (filterType === 'operator' || filterType === 'all') {
                        cardPromises.push((async () => {
                            try {
                                const { infoRes, photoRes } = await window.callApiSearch('fetch_operator_info', { searchWords });
                                ...
                                if (missingInfos.length > 0) {
                                    const { data: extraInfos } = await window.callApiSearch('fetch_operator_extra', { missingInfos });
                                    if (extraInfos) {
                    ...
                    if (filterType === 'model' || filterType === 'all') {
                        cardPromises.push((async () => {
                            try {
                                const { infoRes, vehRes } = await window.callApiSearch('generic_query', { queryType: 'fetch_model_info', params: { searchWords } });
                                ...
                                if (missingInfos.length > 0) {
                                    const { data: extraInfos } = await window.callApiSearch('fetch_model_extra', { missingInfos });
                                    if (extraInfos) {
                        ...
                        const { data: brandLogoData } = await window.callApiSearch('generic_query', { queryType: 'fetch_brand_logo', params: { brandName } });
                    ...
                    if (filterType === 'plate' || filterType === 'model' || filterType === 'all') {
                        cardPromises.push((async () => {
                            try {
                                ...
                                const { data: vData } = await window.callApiSearch('fetch_vehicles', { searchWords, filter: app.preference.current, selectStr });
                    ...
                    const { data: results, error } = await window.callApiSearch('fetch_photos', { filterType, query, searchWords, currentPreference: app.preference.current, prefixToUrl, currentParams: Object.fromEntries(currentParams) });
...
window.app.vehicle = {
                ...
                saveHistory: async () => {
                    ...
                    if(app.role === 'admin' || app.role === 'manager') {
                        try {
                            await window.callApiSearch('save_history', { plate: app.currentPlate, payload });
                        ...
                        try {
                            ...
                            const { count, error: checkErr } = await window.callApiSearch('generic_query', { queryType: 'check_edit_request_history', params: { plate: app.currentPlate } });
                            ...
                            const { error } = await window.callApiSearch('generic_query', { queryType: 'insert_edit_request', params: { reqData } });
                            if (error) throw error;
                ...
                syncHistoryOnPhotoEdit: async (plate, takenAtIso, oldData, newData) => {
                    ...
                    try {
                            const { data: photos } = await window.callApiSearch('generic_query', { queryType: 'fetch_history_photos', params: { plate } });
                        ...
                        if (isSpecial) {
                            if (otherNormalPhotos.length > 0) {
                                ...
                                await window.callApiSearch('generic_query', { queryType: 'update_vehicle_history_item', params: { id: item.id, operator: op, route_no: rt } });
                                await window.callApiSearch('generic_query', { queryType: 'delete_vehicle_history_item', params: { id: item.id } });
                            }
                        } else {
                                const { data: history } = await window.callApiSearch('generic_query', { queryType: 'check_vehicle_history_item', params: { plate, operator: op, route_no: rt } });
                                await window.callApiSearch('generic_query', { queryType: 'update_vehicle_history_dates', params: { id: history.id, effective_date: targetDate } });
                                await window.callApiSearch('generic_query', { queryType: 'insert_vehicle_history_item', params: { license_plate: plate, effective_date: targetDate, operator: newData.operator, route: newData.route_no, display_order: 999 } });
                            }
                        }
                ...
                saveVehiclePageChanges: async (plate) => {
                    ...
                    try {
                        if (app.role === 'admin' || app.role === 'manager') {
                            ...
                            const { error } = await window.callApiSearch('generic_query', { queryType: 'update_vehicle_details', params: { plate, newData } });
                            if (error) throw error;
                            app.toast.show('success', 'Đã lưu thay đổi', 'Thông tin xe đã được cập nhật thành công.');
                            app.views.loadVehiclePage(plate, true);
                        } else {
                            // [BẢO VỆ] Kiểm tra xem đã có yêu cầu nào đang chờ duyệt cho xe này chưa
                            const { count } = await window.callApiSearch('generic_query', { queryType: 'check_edit_request_details', params: { plate } });
                            if (count > 0) {
                                btnSave.disabled = false; btnSave.innerHTML = 'Gửi yêu cầu';
                                return app.ui.showAlert("Có yêu cầu chỉnh sửa hồ sơ khác đang chờ duyệt cho xe này. Vui lòng thử lại sau.");
                            }

                            const { error } = await window.callApiSearch('generic_query', { queryType: 'insert_edit_request_details', params: { plate, model, note } });
                            if (error) throw error;
...
window.app.operator = {
                ...
                openEditPrompt: async () => {
                    ...
                    try {
                        const { data: opInfo } = await window.callApiSearch('generic_query', { queryType: 'fetch_operator_exact', params: { operator_name: app.currentOperator } });
                        if (opInfo) {
                ...
                submitEdit: async () => {
                    ...
                    try {
                        if (app.role === 'admin' || app.role === 'manager') {
                            ...
                            // ==========================================
                            const { error } = await window.callApiSearch('generic_query', { queryType: 'upsert_operator_info', params: { operator_name: app.currentOperator, avatar_url: logo, description: desc } });
                            if (error) throw error;
                            ...
                            // ==========================================
                            const { count, error: checkErr } = await window.callApiSearch('generic_query', { queryType: 'check_edit_request_operator', params: { operator_name: app.currentOperator } });
...
window.app.model = {
                ...
                loadModelPage: async (modelName, forceRefresh = false) => {
                    ...
                    try {
                        ...
                        const { data: exactInfo } = await window.callApiSearch('generic_query', { queryType: 'fetch_model_exact', params: { model_name: modelName } });
                        ...
                        const { data: statsData, error: statsErr } = await window.callApiSearch('generic_query', { queryType: 'fetch_model_stats', params: { modelName } });
                        ...
                        const { data: photos, error } = await window.callApiSearch('generic_query', { queryType: 'fetch_model_photos', params: { modelName, currentPreference: app.preference.current } });
                        if (error) throw error;
                ...
                openEditPrompt: async () => {
                    ...
                    try {
                        ...
                        const { data: exactInfo } = await window.callApiSearch('generic_query', { queryType: 'fetch_model_description', params: { model_name: app.model.currentModel } });
                        ...
                        const { data: brandLogoData } = await window.callApiSearch('fetch_brand_logo', { brandName });
                ...
                submitEdit: async () => {
                    ...
                    try {
                        ...
                        if (app.role === 'admin' || app.role === 'manager') {
                            ...
                            const { error: upsertErr } = await window.callApiSearch('generic_query', { queryType: 'upsert_model_info', params: { model_name: app.model.currentModel, logo_url: logo, description: desc } });
                            ...
                            await window.callApiSearch('generic_query', { queryType: 'update_model_brand_logo', params: { logo_url: logo, brandName } });
                            ...
                            const { count, error: checkErr } = await window.callApiSearch('generic_query', { queryType: 'check_edit_request_model', params: { model_name: app.model.currentModel } });
